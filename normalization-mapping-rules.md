...
> **구현 상태 (예약 필드)**: 현재 타임라인 파이프라인은 `ROAM_SUCCESS` 이벤트를 생성하지 않으므로,
> `server.js`는 이 필드를 항상 `false`로 둔다. 위 조건은 향후 로밍 감지를 도입할 때를 위한 예약 규격이며,
> 지금의 `false` 고정값은 "신호 없음"을 정직하게 나타낸 것이다(미산출 스텁).
