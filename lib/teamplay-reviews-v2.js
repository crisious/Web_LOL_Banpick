...
function frameConfidence(frameAgeSeconds) {
  if (frameAgeSeconds <= 5) return "HIGH";
  if (frameAgeSeconds <= 15) return "MEDIUM";
  return "LOW";
}
...
function positioningFacts(reviewId, scene, encounters, objectives, source) {
  ...
  const frameAgeSeconds = Math.round((anchor.timestamp - frame.timestamp) / 1000);
  const confidence = frameConfidence(frameAgeSeconds);
  ...
  if (death) {
    facts.push(finalizeFact(reviewId, {
      ...
      value: {
        ...
        secondsAfterCapture: Math.round(
