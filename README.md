...
```bash
npm test                 # 모든 테스트 일괄 실행 (test-artifacts/run-tests.mjs · 총 595건)
...
`QA` workflow를 수동 실행할 때 `external_readonly_url`에 외부 `https://` 데모 URL을 입력하면 `npm run smoke:report:external:readonly` 결과도 같은 artifact에 포함됩니다. manual external URL은 workflow 초반에 사전 검증되어 unit/local smoke 전에 빠르게 실패하며, 공개 FQDN 또는 public IP literal이어야 합니다. DNS hostname label은 알파벳/숫자/hyphen만 사용할 수 있고 hyphen으로 시작하거나 끝날 수 없습니다. username/password, query string, fragment, localhost, loopback, single-label hostname, DNS-incompatible hostname label, private/internal IP target, IPv4-mapped IPv6 private/internal target을 포함하면 실패합니다. `external_protected_url`은 `PUBLIC_DEMO_TOKEN` secret이 설정되어 있을 때만 `npm run smoke:report:external:protected`를 실행합니다. `external_protected_url`을 입력했지만 token secret이 없으면 workflow는 `external_protected_url requires repository secret PUBLIC_DEMO_TOKEN` 메시지로 실패합니다. 외부 URL 입력값은 step env로 넘기고 shell에서는 quoted variable로 사용합니다.
