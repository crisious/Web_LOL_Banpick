...
`smoke:report:*` runner는 `--token=`, `--timeout-ms=`, sample manifest error expectation 계열만 smoke pass-through 옵션으로 허용합니다. `--expectmode=readonly` 같은 오타나 runner가 직접 생성하는 `--expect-mode=<mode>`를 수동으로 넘기면 report 디렉터리나 메타데이터를 만들기 전에 `FAIL unknown smoke report option: ...`로 종료합니다. 허용된 pass-through 옵션도 runner 단계에서 singleton/value/필수 조합을 검증하므로 `--timeout-ms=0`, 중복 `--timeout-ms=...`, sample manifest expectation의 누락된 id/code/status 오류 역시 artifact 생성 전에 실패합니다. Sample detail error id는 `sample-[a-z0-9-]+`, sample error code는 `[A-Z0-9_]+` 형식만 허용해 URL이나 공백 포함 값이 네트워크 요청 또는 artifact 생성 전에 차단됩니다.
