...
```bash
npm test                 # 모든 테스트 일괄 실행 (test-artifacts/run-tests.mjs · 총 649건)
...
`QA` workflow를 수동 실행할 때 `external_readonly_url`에 외부 `https://` 데모 URL을 입력하면 `npm run smoke:report:external:readonly` 결과도 같은 artifact에 포함됩니다. manual external URL은 workflow 초반에 사전 검증되어 unit/local smoke 전에 빠르게 실패하며, direct `smoke:external:*` / `smoke:external:manifest:*` 명령과 `smoke:report:external:*` runner도 같은 preflight를 smoke 실행 전에 적용합니다. 공개 FQDN 또는 publicly routable IP literal의 origin/root URL이어야 합니다. raw DNS hostname label은 ASCII 알파벳/숫자/hyphen만 사용할 수 있고 hyphen으로 시작하거나 끝날 수 없습니다. IDN은 `xn--` punycode label을 직접 입력한 경우만 허용합니다. username/password, query string, fragment, raw ASCII control character, unencoded space, literal backslash, path dot segment, path other than `/`, explicit non-default port, localhost, loopback, single-label hostname, DNS-incompatible hostname label, parser-normalized raw hostname spelling, non-canonical IPv4 literal spelling, private/internal IP target, IPv4-mapped IPv6 private/internal target, documentation/benchmarking/multicast/reserved/special-use IP literal, IANA special-purpose IPv4 literal, IANA special-purpose/reserved IPv6 literal을 포함하면 실패합니다. `external_protected_url`은 `PUBLIC_DEMO_TOKEN` secret이 설정되어 있을 때만 `npm run smoke:report:external:protected`를 실행합니다. `external_protected_url`을 입력했지만 token secret이 없으면 workflow는 `external_protected_url requires repository secret PUBLIC_DEMO_TOKEN` 메시지로 실패합니다. 외부 URL 입력값은 step env로 넘기고 shell에서는 quoted variable로 사용합니다.
