...
Public demo 관련 `.env` 값은 `=` 뒤 공백까지 그대로 읽습니다. 예를 들어 `PUBLIC_DEMO_MODE= readonly`나 `PUBLIC_DEMO_TOKEN=secret `은 보정되지 않고 `/healthz`의 invalid config 진단과 smoke 실패로 드러납니다. `TRUST_PROXY`도 exact `1`일 때만 켜지며, `TRUST_PROXY= 1`처럼 공백이 섞인 값은 proxy header를 신뢰하지 않고 socket IP 기준 rate-limit를 유지합니다.
