...
```bash
npm test                 # 모든 테스트 일괄 실행 (test-artifacts/run-tests.mjs · 총 545건)
...
`QA` workflow를 수동 실행할 때 `external_readonly_url`에 외부 `https://` 데모 URL을 입력하면 `npm run smoke:report:external:readonly` 결과도 같은 artifact에 포함됩니다. `external_protected_url`은 `PUBLIC_DEMO_TOKEN` secret이 설정되어 있을 때만 `npm run smoke:report:external:protected`를 실행합니다. 외부 URL 입력값은 step env로 넘기고 shell에서는 quoted variable로 사용합니다.
