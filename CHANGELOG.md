...
**총 페이즈**: 24 (1~9 패치 / 10 문서화 / 11 데이터 연결 / 12 정리 / 13~18 폴리싱·확장 / 20 비평·계획 / 21~24 비평 후속)
...
## Phase 20 — 디자인 비평 + 후속 작업 계획

**파일**: `PLAN.md` (신규)

`design:design-critique` 스킬로 `_design-mockups/improved-full.html`을 자체 비평. 9개 finding을 영향도/노력 매트릭스로 분류해 4개 실행 페이즈로 그룹핑하고 PLAN.md에 기록. 명시적으로 SKIP/DEFER한 항목(폰트 16px 전환, admin 토큰 정리 등)도 사유와 함께 문서화.

## Phase 21 — 시각 경합 해소 (P1)

**파일**: `styles.css`, `_design-mockups/improved-full.html`

**21a**: 히어로(amber-mint 듀얼 그라디언트)와 score-panel(conic-gradient 링 + 막대)이 같은 시각 무게로 경합하던 문제. score-panel 자체에 명시적 패딩·보더·`var(--surface-1)` 약한 배경을 부여해 panel이 콘텐츠 컨테이너로 전환되도록 함. 히어로의 amber 그라디언트가 "유일한 특별 영역" 자격을 회복.

**21b**: 추세 탭 상단 `.trend-callout`이 히어로와 동일한 amber 그라디언트를 써서 의미 시그널이 약화되던 문제. 목업의 `.trend-callout` 배경을 `linear-gradient(135deg, var(--tint-info), transparent 60%)` + `var(--info-border)`로 변경. 추세 = 시간 기반 정보라는 의미와 푸른 톤(info)이 일치.

## Phase 22 — 사용성 마찰점 정리 (P2)

**파일**: `styles.css`, `index.html`, `_design-mockups/improved-full.html`

**22a**: 빌드 오더 데스크톱 가로 스크롤바 노이즈. 6개 빌드 아이템이 데스크톱에서 한 줄에 충분히 들어가는데 `overflow-x: auto`로 스크롤바가 항상 노출되던 문제. `flex-wrap: wrap`을 기본으로 하고 모바일(<720px)에서만 `overflow-x: auto`로 가로 스크롤 폴백.

**22b**: 사이드바 fetcher `<details>`가 접혀 있어 첫 사용자가 발견 어려운 문제. summary `::before` 화살표를 `var(--accent)` 색 + `var(--fs-md)` 사이즈로 강화하고, summary 자체에 hover 시 `var(--surface-2)` 배경 추가해 클릭 가능 영역임을 명확히. 패딩도 키워 터치 타겟 확장.

**22c**: 메트릭 카드 첫 카드 강조가 `:first-child`에 의존해 렌더 순서가 바뀌면 의도치 않은 카드가 강조될 위험. CSS의 `:first-child` 폴백 제거하고 명시 `.metric--primary` 클래스만 적용. `index.html`의 CS/분 카드(delta가 붙는 유일한 메트릭)에 `class="metric metric--primary"` 직접 부여.

## Phase 23 — 접근성 + 일관성 폴리싱 (P3)

**파일**: `_design-mockups/improved-full.html`

**23a**: SVG KDA 차트가 `aria-label`만 있어 시각 장애 사용자가 트렌드 컨텍스트를 알 수 없던 문제. `<title id="kda-chart-title">` + `<desc id="kda-chart-desc">`로 그래프 패턴(0분→28분 점진 상승, 9분 첫 킬·16분 더블킬·22분 바론에서 가속) 명시적 설명. `role="img" aria-labelledby="kda-chart-title kda-chart-desc"`로 두 요소를 묶음.

**23b**: 점수 링이 항상 mint(78점) 단일 변형만 보여 디자이너가 다른 점수대 시각 검증 불가. 추세 탭 상단에 `.score-states` 미니 갤러리 추가 — 88(mint·우수) / 72(mint·양호) / 58(amber·평균) / 38(rose·개선 필요) 4개 변형을 80×80 컴팩트 링으로 시각화. `--ring-color` CSS custom prop으로 색 변형 제어.

**23c**: 인사이트 헤딩의 ✓/! 아이콘이 목업에선 inline `<span aria-hidden="true">`, 프로덕션 styles.css에선 `::before` 자동 prefix로 일관성 없던 문제. 목업도 ::before 방식으로 통일하고 inline span 제거. 마크업이 깔끔해지고 디자이너 의도가 CSS 한 곳으로 집중.

## Phase 24 — insight-item 중첩 박스 무게 조정 (P4)

**파일**: `_design-mockups/improved-full.html`

`.insight-panel`(var(--space-5) padding) 안에 `.insight-item`(var(--space-3) padding + 1px 보더 + surface-3 배경)이 중첩되어 시각적으로 무거운 박스-안의-박스 느낌. insight-item의 `border: 1px solid var(--surface-3)`만 제거하고 `var(--surface-1)` 배경은 유지 — 아이템 분리는 배경 색 차이만으로 충분, 패널 내부가 한결 가벼워짐. (본문 폰트 16px 전환은 영향 범위 큼 — Phase 24 옵션 A만 수행, 옵션 B는 PLAN.md에 DEFER로 기록)
...
| 19 | 종합 검증 + git diff stat 갱신 | JS 4/4 + CSS 7/7 + axe 0 violations |
| 20 | design-critique 실행 + PLAN.md 작성 | 9 finding → 4 페이즈 매트릭스 |
| 21 | 시각 경합 해소 (히어로 ↔ score panel + trend-callout 톤) | 시각 위계 명확화 |
| 22 | 사용성 마찰 제거 (빌드 nowrap + fetcher 발견성 + primary 명시) | 3개 마찰점 정리 |
| 23 | a11y + 일관성 폴리싱 (SVG title/desc + 점수 변형 갤러리 + 아이콘 통일) | 디자이너/개발자 둘 다 이득 |
| 24 | insight-item 중첩 보더 제거 | 패널 내부 시각 무게 경량화 |
