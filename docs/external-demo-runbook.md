...
- **Refusals fall back to another model automatically.** Opus 5 runs safety classifiers that can decline a request outright — HTTP 200 with `stop_reason: "refusal"` — and benign work occasionally trips them. Requests opt into the server-side `fallbacks: "default"` parameter (beta `server-side-fallback-2026-07-01`), so a declined request is re-run server-side on Anthropic's recommended model for that refusal category, inside the same call. Three things follow from that:
  - **The substitute model bills at its own rates.** A declined attempt that produced no output is not billed; a mid-stream refusal bills the partial.
  - **A substitution is never silent.** The server logs `[AI] Claude request fell back: <from> declined, <to> served`. Check for that line before attributing an output-quality change to Opus 5.
  - **`AGENT_DISABLE_API_FALLBACK=1` turns the opt-in off**, exactly like `AGENT_DISABLE_CODEX=1` (an exact `1`; ` 1` or `true` leave it on). Without the opt-in a refusal simply raises and the run drops to the rule-based fallback chain.

  If the beta is not enabled on the account, no configuration is needed: the client sees the 400, drops the opt-in, and retries once without it. That downgrade does not consume the 5xx retry budget. The parameter is Claude-API-only — this client posts to `api.anthropic.com` directly, so that restriction does not bite here, but it would on Bedrock, Vertex, or Foundry.
