...
Direct smoke also rejects unknown `--...` options before any network request or report JSON write. This keeps typos such as `--expectmode=readonly` from silently falling back to a weaker smoke configuration. Sample manifest error expectation probes accept only `sample-[a-z0-9]+(-[a-z0-9]+)*` sample detail ids and `[A-Z0-9_]+` diagnostic codes, so URL-like values, whitespace-containing values, or ids with empty hyphen segments fail before network probes.
...
The `smoke:report:*` runner only forwards allowlisted smoke pass-through options: `--token=`, `--timeout-ms=`, and sample manifest error expectation options. Unknown options or runner-owned smoke options such as `--expect-mode=<mode>` fail in the runner before report directories or metadata files are created. Allowlisted pass-through options are also checked for singleton, value, required-field, and token-shape contracts in the runner, so invalid values such as `--timeout-ms=0`, duplicate timeout flags, incomplete sample manifest error expectations, URL-like sample detail ids, ids with empty hyphen segments, or whitespace-containing diagnostic codes fail before artifact creation.
