...
The `smoke:report:*` runner only forwards allowlisted smoke pass-through options: `--token=`, `--timeout-ms=`, and sample manifest error expectation options. Unknown options or runner-owned smoke options such as `--expect-mode=<mode>` fail in the runner before report directories or metadata files are created. Allowlisted pass-through options are also checked for singleton, value, required-field, token-shape, and HTTP error status contracts in the runner, so invalid values such as `--timeout-ms=0`, duplicate timeout flags, incomplete sample manifest error expectations, URL-like sample detail ids, ids with empty hyphen segments, whitespace-containing diagnostic codes, or sample error statuses outside `400-599` fail before artifact creation.
