...
### padding·margin 처리 (2026-07-27 완료)

시작 상태: padding 24.8%(raw 112) / margin 11.1%(raw 88).

**1. 감사 오탐 제거** (`fea122a`) — `margin: 0 auto`와 `margin-bottom: 0 !important`가 raw로 집계됐다. `keywordOk`를 값 전체와 통짜로 비교해 축이 둘인 `0 auto`가 걸리지 않았다. 축 단위 검사로 바꾸고 `!important`를 떼도록 했다. margin raw 88 → 85.

**2. 기계적 치환 80건** (`1d97a1b`, `cfac298`) — 기존 `--space-*` 값과 정확히 일치하는 단일값 50건, 모든 축이 토큰값인 shorthand 17건, 키워드가 섞인 shorthand 13건. shorthand의 축 순서는 보존했다(`12px 10px` → `var(--space-12) var(--space-10)`).

**3. 래더를 값 기반 이름으로 재편하고 확장** (`45b28d8`) — 서수 이름의 근본 문제는 값 사이에 새 값을 넣을 수 없다는 것이다. `6px`을 추가하려면 `--space-1`(4px)과 `--space-2`(8px) 사이에 자리가 없어 그 위 전부를 재번호해야 한다. 이름의 숫자를 px 값으로 바꿨다.

```
--space-0(2px)  -> --space-2       신설: --space-6  (6px)
--space-1(4px)  -> --space-4             --space-14 (14px)
--space-2(8px)  -> --space-8             --space-18 (18px)
--space-3(10px) -> --space-10            --space-20 (20px)
--space-4(12px) -> --space-12            --space-24 (24px)
--space-5(16px) -> --space-16
--space-6(22px) -> --space-22
```

신설 5개는 남은 값의 축 단위 빈도에서 골랐다(14px×27, 18px×22, 6px×17, 24px×12, 20px×8). 이 5개로 부분 shorthand(`padding: 10px 14px`처럼 한 축만 토큰이던 것)가 함께 풀려 추가 75건을 치환했다.

**결과: padding 24.8% → 82.6%(raw 26) / margin 11.1% → 83.3%(raw 16).** `spacing`(gap)은 92.9%로 불변.

**검증** — 재편은 단일 패스 매핑으로 했다(순차 치환하면 `--space-0`→`--space-2`와 `--space-2`→`--space-8`이 겹쳐 이중 적용된다). `var(--space-*)` 참조 308회의 환산값, 그리고 padding/margin/gap 선언 453개의 계산된 최종 값이 재편·치환 전후 전부 동일함을 확인했다(별칭 재귀 전개 후 대조). 픽셀 변화 없음.

### 남은 것 — 여기서 멈춘 이유

padding raw 26건 / margin raw 16건. 남은 값의 성격이 래더와 구조적으로 다르다.

- **홀수 소값**: `3px`×8, `5px`×6, `7px`×4, `9px`×2 — `padding: 5px 10px` 같은 광학적 조정으로 보인다.
- **큰 이상치**: `26px`, `28px`×3, `32px`, `36px`, `44px`×2, `48px`, `96px` — 대부분 단발, 레이아웃 특정.
- **rem 단위**: `0.1rem`, `0.25rem`, `0.5rem` — px 래더에 섞는 것은 별개 결정.
- **음수**: `-1px`, `-4px`, `-6px`, `-7px` — 겹침 조정용.

값 기반 이름이므로 `--space-3` 삽입은 문법적으로 자유롭다. 그럼에도 멈춘 이유는 **신호 보존**이다. 2~10의 모든 정수에 토큰을 주면 래더가 제약이 아니라 픽셀 별칭이 되고, 제약되지 않은 선택이 감사에서 보이지 않게만 된다. 남은 값이 raw로 보이는 것이 정직하다.

계속하려면 빈도 순으로 `3px`(8회)·`5px`(6회)·`7px`(4회)가 다음 후보다.
