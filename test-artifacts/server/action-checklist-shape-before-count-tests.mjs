// server.js buildAnalysis actionChecklist shape-before-count regression tests
...
const buildAnalysis = new Function("state", `
  ...
  function buildActionChecklist() {
    state.actionRepairCalls += 1;
    return [
      { id: "act_1", text: "repaired action 1" },
      { id: "act_2", text: "repaired action 2" },
      { id: "act_3", text: "repaired action 3" },
    ];
  }
...
function normalizedFixture() {
  return {
    matchInfo: { matchId: "KR_ACTION_SHAPE_BEFORE_COUNT", queueLabel: "RANKED_SOLO" },
    playerContext: { riotId: "Action#KR1", participantId: 1 },
    timelineEvents: [],
    phaseContext: {},
    playerStats: {},
    teamContext: {},
...
const result = await buildAnalysis(normalizedFixture(), "sample-action-shape-before-count");
